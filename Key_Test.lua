...
do
 ...
 ----------------------------------------------------------------
    ...
    local function startMainHub(keydata, Library)
        ...
        -- [Dynamic Token Gen] สร้าง Token: ความลับ + วันที่ (เช่น BxB_Secret_20251207)
        local secretSalt = "BxB_SUPER_SECRET_SALT_CHANGE_THIS" 
        local datePart = os.date("%Y%m%d") -- จะได้ค่าเป็นปีเดือนวัน เช่น 20231025
        local dynamicToken = secretSalt .. "_" .. datePart

        -- ส่ง dynamicToken ไปแทน Token เดิม
        local success, err2 = pcall(startFn, Exec, keydata, dynamicToken)
