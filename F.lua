...
local function parseChangelogBody(body, HttpService)
    ...
    -- [SECURITY] Real-time Expiration Check with Grace Period
    task.spawn(function()
        local graceStarted = false
        local graceDuration = 600 -- 10 Minutes
        ...
        while true do
            ...
            if expireTs > 0 then
                local timeLeft = expireTs - now
                
                -- แจ้งเตือนเมื่อใกล้หมด (เหลือ 5 นาที)
                if timeLeft <= 300 and timeLeft > 0 and timeLeft % 60 == 0 then
                    Library:Notify("Warning: Key expires in " .. math.ceil(timeLeft/60) .. " minutes!", 5)
                end

                -- เข้าสู่โหมด Grace Period
                if timeLeft <= 0 then
                    if not graceStarted then
                        graceStarted = true
                        graceEndTime = now + graceDuration
                        Library:Notify("KEY EXPIRED! Grace Period Active (+10 Mins)", 10)
                        -- แจ้งเตือนใหญ่ๆ (ถ้าทำได้) หรือส่งเสียง
                    end
                    
                    local graceLeft = graceEndTime - now
                    if graceLeft <= 0 then
                        -- หมดเวลา Grace Period -> ทำลายระบบ
                        Library:Notify("SESSION TERMINATED. GOODBYE.", 10)
                        task.wait(2)
                        if Library.Unload then Library:Unload() end
                        script:Destroy() -- ทำลายตัวเอง
                        break
                    elseif graceLeft % 60 == 0 then
                        Library:Notify("Grace Period: " .. math.ceil(graceLeft/60) .. " mins left", 3)
                    end
                end
            end
        end
...
-- [UPDATED] Auto Run Pattern
    local AutoRunToggle = MoveBox:AddToggle("bxw_autorun", { Text = "Auto Run", Default = false })
    local AutoRunPattern = MoveBox:AddDropdown("bxw_autorun_pat", { Text = "Pattern", Values = {"Circle", "Square", "ZigZag"}, Default = "Circle", Multi = false })
    local autoRunConn
        ...
        if state then
            ...
            autoRunConn = AddConnection(RunService.Heartbeat:Connect(function(dt)
                ...
                if hum then
                    ...
                    local pattern = AutoRunPattern.Value
                    
                    if pattern == "Circle" then
                        offset = Vector3.new(math.cos(t)*10, 0, math.sin(t)*10)
                    elseif pattern == "Square" then
                        local size = 10
                        local phase = math.floor(t % 4)
                        if phase == 0 then offset = Vector3.new(t%1*size, 0, 0)
                        elseif phase == 1 then offset = Vector3.new(size, 0, t%1*size)
                        elseif phase == 2 then offset = Vector3.new(size - t%1*size, 0, size)
                        else offset = Vector3.new(0, 0, size - t%1*size) end
                    elseif pattern == "ZigZag" then
                         offset = Vector3.new(t*5, 0, math.sin(t*5)*5)
                    end
    ...
    -- [NEW] Spider-Man Wall Climb
    local SpiderToggle = MoveBox:AddToggle("bxw_spiderman", { Text = "Spider-Man (Wall Climb)", Default = false })
    local spiderConn
    SpiderToggle:OnChanged(function(state)
        if state then
            spiderConn = AddConnection(RunService.Heartbeat:Connect(function()
                local char = getCharacter()
                local root = getRootPart()
                local hum = getHumanoid()
                if char and root and hum then
                    ...
                    if hit then
                        ...
                        vel.Velocity = Vector3.new(0, 20, 0) -- ปีนขึ้น
                    else
    ...
    local spectateLoop
 -- [FIXED] Spectate Logic (แก้กล้องสั่น/วาป)
    SpectateToggle:OnChanged(function(state)
        ...
        if state then
             ...
             if target then
                 -- Force Reset Camera Type to avoid conflict with FreeCam
    ...
    UtilBox:AddButton("Set Waypoint", function()
        ...
        if not name or name == "" or name == "Point" then 
            name = "Point_" .. wpCount 
            wpCount = wpCount + 1
        end
        
        -- (Logic เดิมในการบันทึก)
                            ...
                            if not onScreen then
                                ...
                                if HealthToggle.Value then
                                    ...
                                    if not data.Health then 
                                        ...
                                        data.Health.Outline.Thickness = 4 
                                        data.Health.Bar.Thickness = 2
                                ...
                                if InfoToggle.Value then
                                    ...
                                    data.Info.Size = 13
                                    data.Info.Text = string.format("[%s] %s [%d HP]", teamName, plr.DisplayName, hp, "\n"..item)
                                    data.Info.Position = Vector2.new((minX + maxX) / 2, maxY + 15)
                                else
                                    if data.Info then data.Info.Visible = false end
                                end
                                -- [FIXED] Head Dot Logic
                                ...
                                if not data.HeadDot then 
                                    ...
                                    data.HeadDot.Radius = 4
                                end
                                    data.HeadDot.Visible = true
                                    data.HeadDot.Position = Vector2.new(screenPos.X, screenPos.Y)
                                    data.HeadDot.Color = Options.bxw_esp_headdot_color.Value
                                else
                                    if data.HeadDot then data.HeadDot.Visible = false end
                                end
                               end
...
-- [NEW] Look Aim Indicator (Text Warning)
    if TargetIndToggle.Value then
        local head = char:FindFirstChild("Head")
        if head then
            local toMe = (cam.CFrame.Position - head.Position).Unit
            local look = head.CFrame.LookVector
            local dot = toMe:Dot(look)
            
            if dot > 0.8 then -- ถ้าหันมาทางเราเกิน 80%
                if not data.LookWarn then 
                    data.LookWarn = Drawing.new("Text")
                    data.LookWarn.Center = true
                    data.LookWarn.Color = Color3.new(1,0,0)
                    data.LookWarn.Size = 18
                    data.LookWarn.Outline = true
                    data.LookWarn.Text = "WARNING: AIMING AT YOU!"
                end
                data.LookWarn.Visible = true
                data.LookWarn.Position = Vector2.new(workspace.CurrentCamera.ViewportSize.X/2, workspace.CurrentCamera.ViewportSize.Y/2 - 100)
            else
                if data.LookWarn then data.LookWarn.Visible = false end
            end
        end
    end
...
-- [FIXED] Whitelist Logic
    ...
    do
        ...
        local CustomHeadChance = AimBox:AddSlider("bxw_custom_head", { Text = "Custom Head %", Default = 50, Min = 0, Max = 100, Rounding = 0 })
    local CustomBodyChance = AimBox:AddSlider("bxw_custom_body", { Text = "Custom Body %", Default = 50, Min = 0, Max = 100, Rounding = 0 })
                                ...
                                if hum and hum.Health > 0 then
                                    ...
                                    if rootCandidate then
                                        ...
                                        if not skip then
                                            ...
                                            if aimPartName == "Custom" then
        local rnd = math.random(1, 100)
        if rnd <= CustomHeadChance.Value then
            selectedPart = char:FindFirstChild("Head")
        else
            selectedPart = char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso")
        end
                                            elseif aimPartName == "Head" then selectedPart = char:FindFirstChild("Head")
...
-- [UPDATED] Server Browser (Fixed Groupbox Error & Join Button Logic)
    ...
    ServerBrowserBox:AddButton("Refresh Servers", function()
        Library:Notify("Fetching servers... (This may take a while)", 3)
        serverList = {}
        serverObjs = {}
            ...
            if ok then
                ...
                if json and json.data then
                    ...
                    table.sort(serverObjs, function(a, b)
                        ...
                        if sort == "Highest Players" then return a.playing > b.playing end
                        if sort == "Lowest Ping" then return (a.ping or 999) < (b.ping or 999) end
                    ...
                    Library:Notify("Found " .. #serverList .. " servers.", 3)
                else
