...
local function MainHub(Exec, keydata, authToken)
    ...
    -- Crosshair lines storage. This will hold drawing objects for the optional
    -- crosshair overlay so they can be cleaned up on unload. It is defined
    -- here so that it is captured by the OnUnload closure later.
    ...
    -- Attempt to fetch additional key details from the remote key server JSON.  The file is
    -- stored in this repository at "Key_System/data.json" on the main branch.  If found, it
    ...
    -- Provide a button to copy key information to the clipboard. This makes it easy for
    -- users to send their key details (masked) to support or for record keeping.
    KeyBox:AddDivider()
    KeyBox:AddButton("Copy Key Info", function()
        -- Determine the plain (non HTML) role and status fields
        ...
        -- Expiration display is already computed above (expireDisplay)
   ...
   local PlayerTab = Tabs.Player
    ...
    -- Movement presets
    -- A dropdown allowing the user to quickly set WalkSpeed and JumpPower to preset values.
    ...
    MovePresetDropdown:OnChanged(function(value)
        if value == "Default" then
            WalkSpeedSlider:SetValue(defaultWalkSpeed)
            JumpPowerSlider:SetValue(defaultJumpPower)
            -- disable toggles so defaults are applied
            WalkSpeedToggle:SetValue(false)
            JumpPowerToggle:SetValue(false)
    ...
    -- FOV settings moved to Camera & World group (Player tab)
    ...
    -- Camera & World settings group moved from ESP tab
    do
    ...
    do
        ...
        -- Smart ESP: Only render visible parts and color them accordingly
        ...
        -- Head Dot feature: draw a small dot on the head of each player
        ...
        -- AddColorPicker must be called on an object with a TextLabel, such as a label or toggle.
        -- Create labels as hosts for each color picker to avoid "missing method" errors.
        ...
        -- ESP refresh interval slider (in milliseconds)
        ...
        -- (Camera & World settings moved to Player tab)

        -- Crosshair overlay controls
        -- These options let the user draw a simple crosshair at the center of the screen.
        ...
        ------------------------------------------------
        local espDrawings = {}
        -- Track last update time for ESP refresh rate
        ...
        local function updateESP()
            -- Throttle ESP updates using user-configurable refresh interval
            ...
            for _, plr in ipairs(Players:GetPlayers()) do
                ...
                if plr ~= LocalPlayer or (SelfToggle and SelfToggle.Value) then
                    ...
                    -- If no humanoid/root or dead, hide any existing ESP and skip processing
                    ...
                    elseif hum and hum.Health > 0 and root then
                        ...
                        if skipPlayer then
                            ...
                            if ChamsToggle.Value then
                                ...
                                -- Use Options table to get actual ColorPicker values (AddColorPicker returns parent label)
                            ...
                            if not onScreen then
                                ...
                                -- default box color from Options
                                ...
                                if HealthToggle.Value then
                                    ...
                                    if Options.bxw_esp_health_color then
                                        bar.Color = Options.bxw_esp_health_color.Value
                                    else
                                ...
                                -- Head dot: draw a small circle on the head of each visible player
        ...
        --[[
            Setup cleanup on player death. When a player's humanoid dies, remove their ESP drawings immediately.
            Maintain a table of connections so they can be disconnected when players leave or the UI unloads.

            This logic was previously placed outside of the MainHub function which caused issues such as undefined
            variables. Moving it here ensures it has access to removePlayerESP and AddConnection.
        --]]
        do
            local deathConnections = {}

            -- Helper to attach a death listener to a player's humanoid
            local function attachDeathListener(plr)
                ...
                if hum then
                    ...
                    if deathConnections[plr] then
                        pcall(function()
                            deathConnections[plr]:Disconnect()
                        end)
                        deathConnections[plr] = nil
                    end
                    -- Connect the Died event using AddConnection for cleanup
            ...
            -- Attach death listeners to existing players
            for _, plr in ipairs(Players:GetPlayers()) do
                attachDeathListener(plr)
                -- Listen for future character spawns for this player
                AddConnection(plr.CharacterAdded:Connect(function()
                    attachDeathListener(plr)
                end))
            end
            -- Attach listener for players joining
            AddConnection(Players.PlayerAdded:Connect(function(plr)
                AddConnection(plr.CharacterAdded:Connect(function()
                    attachDeathListener(plr)
                end))
            end))
            -- Clean up death connections when players leave
            AddConnection(Players.PlayerRemoving:Connect(function(plr)
                if deathConnections[plr] then
                    pcall(function()
                        deathConnections[plr]:Disconnect()
        ...
        -- Create the drawing objects once. They will be updated every frame
        -- in the RenderStepped connection below. If the user toggles
        -- crosshair on, these lines draw a simple crosshair at the centre of
        -- the screen. Colour, size and thickness are configurable via
        -- Crosshair settings in the ESP tab.
        crosshairLines = {
            h = Drawing.new("Line"),
            v = Drawing.new("Line"),
        }
        ...
        -- Frame update for crosshair overlay
    ...
    do
        ...
        -- Aim activation mode (Hold Right Click or Always)
        ...
        AddConnection(RunService.RenderStepped:Connect(function()
            ...
            if Toggles.bxw_aimbot_enable and Toggles.bxw_aimbot_enable.Value then
                ...
                if activation == "Always On" or (activation == "Hold Right Click" and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2)) then
                    -- find best target based on FOV and user-defined mode
                    local bestPlr = nil
                            ...
                            if plr ~= LocalPlayer then
                                ...
                                if hum and hum.Health > 0 then
                                    ...
                                    if rootCandidate then
                                        -- skip same team if team check enabled
                                        local skip = false
                                        ...
                                        if not skip then
                                            ...
                                            elseif aimPartName == "Closest" then
                                                -- Choose the part whose screen-projected position is closest to the crosshair
    ...
    do
        ...
        -- (Removed) Unload UI button moved to Settings tab
    end
    ...
    do
        ...
        -- Create a menu group for UI-related toggles and keybinds
        local MenuGroup = SettingsTab:AddLeftGroupbox("Menu", "wrench")

        -- Toggle to open/close the keybind menu
        ...
        -- Toggle for showing a custom cursor
        ...
        -- Dropdown to choose which side notifications appear on
        ...
        -- Dropdown to adjust DPI scaling percentage
        ...
        -- Label and key picker for menu toggle keybind
        ...
        -- Buttons to unload and reload the UI
        MenuGroup:AddButton("Unload UI", function()
            -- Unload the entire UI and stop all functions
            pcall(function()
                Library:Unload()
            end)
        end)
        MenuGroup:AddButton("Reload UI", function()
            -- Reloading unloads and instructs the user to re-run the script
            pcall(function()
                Library:Unload()
            end)
            pcall(function()
        ...
        -- Use the custom keybind for toggling the entire menu. We must set this
        -- after creating the MenuKeybind control so Options.MenuKeybind exists
        Library.ToggleKeybind = Options.MenuKeybind

        -- Attach our library to ThemeManager and SaveManager for theme and config
        ThemeManager:SetLibrary(Library)
        SaveManager:SetLibrary(Library)

        -- We do not want configs to save theme settings (the theme system
        -- manages those separately). Ignore theme keys and our custom menu key.
        SaveManager:IgnoreThemeSettings()
        SaveManager:SetIgnoreIndexes({ "MenuKeybind", "Key Info", "Game Info" })

        -- Define a base folder for storing themes and configurations. Both managers
        -- will use this folder. The folder name is unified across Key UI and
        -- MainHub so that all settings persist in the same location.
        local configFolder = "BxB.Ware_Setting"
        ThemeManager:SetFolder(configFolder)
        SaveManager:SetFolder(configFolder)

        -- Build the configuration section on the tab itself. This adds a
        -- sub-group where the user can save, load, and delete configs.
        SaveManager:BuildConfigSection(SettingsTab)
        -- Apply the theme selection UI to the entire tab. This creates a
        -- theme picker with built‑in presets and allows custom colour editing.
        ThemeManager:ApplyToTab(SettingsTab)

        -- Auto‑load any configuration that has been marked for autoload. If no
        -- config is marked as autoload, this call does nothing.
    ...
    Library:OnUnload(function()
        -- disconnect all recorded connections
        for _, conn in ipairs(Connections) do
            pcall(function()
                conn:Disconnect()
            end)
        end
        -- remove crosshair drawing objects if they exist
