...
local function MainHub(Exec, keydata, authToken)
    ...
    --------------------------------------------------------
    -- 2. PLAYER TAB (Original Logic Preserved + Risky Mark)
    --------------------------------------------------------
   local PlayerTab = Tabs.Player
    ...
    ------------------------------------------------
    -- 4.3 ESP & Visuals Tab (Major Updates: Unload/Ghost Fix, Info Upgrade, Wall Check Sync)
    ------------------------------------------------
    do
        ...
        -- [FIX] Smart ESP Toggle Removed (Use Skeleton + Wall Check Sync instead)
                        ...
                        if skipPlayer then
                            ...
                            if not onScreen then
                                ...
                                if HealthToggle.Value then
                                    ...
                                    local hbX = minX - 6
                                    local barY2 = minY + (maxY - minY) * (1 - (hum.Health / math.max(hum.MaxHealth, 1)))
                                ...
                                if InfoToggle and InfoToggle.Value then
                                    ...
                                    -- Updated Text Format
                                    data.Info.Text = string.format("[HP: %d] [Dist: %.0f%s] [Team: %s]", hum.Health, distNumInfo, suffixInfo, teamName)
    ...
    ------------------------------------------------
    -- 4.4 Combat & Aimbot Tab (Updated for Smart Aim)
    ------------------------------------------------
    do
        ...
        local SmartAimToggle = AimBox:AddToggle("bxw_aim_smart", { Text = "Smart BodyAim", Default = false, Tooltip = "Aim at head if body blocked" }) -- Renamed slightly to distinguish
