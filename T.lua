...
local function MainHub(Exec, keydata, authToken)
   ...
   local MoveBox = PlayerTab:AddLeftGroupbox("Player Movement", "user")
    ...
    do
        ...
        local SmoothSlider = AimBox:AddSlider("bxw_aim_smooth", { Text = "Aimbot Smoothness", Default = 0.5, Min = 0, Max = 5, Rounding = 2, Tooltip = "0=Instant, 5=Slow" })
                    ...
                    if bestPlr then
                        ...
                        if math.random(0, 100) <= HitChanceSlider.Value then
                            ...
                            if AimMethodDropdown.Value == "MouseDelta" then
                                ...
                                local smooth = SmoothSlider.Value
                                if smooth <= 0.05 then -- Treat near 0 as instant
                                    cam.CFrame = newCF
                                else
                                    -- Adjusted formula: Higher smooth value = slower interpolation
    ...
    do
        ...
        -- [BUG FIX] Check if TextLabel exists before setting
        if not isMobile then
             pcall(function() SetPointBtn.TextLabel.Text = "Set Point (Mobile Only)" end)
        end
